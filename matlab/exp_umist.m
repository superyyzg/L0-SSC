...
nK = length(K);
perf_umist = cell(1,nK);
for i = 1:nK,
    k = K(i);
    [data_k_class,tlabel_k_class]  = choose_k_class(data,tlabel,k);
    
    [perf_umist{i},l1graph_alpha,l0graph_alpha] = run_perf_comp(data_k_class,k,tlabel_k_class,km_opt,lambda,lambda_l0graph,maxIter_l1graph_admm,maxIter_l0graph,smce_kmax,omp_Ts,...
