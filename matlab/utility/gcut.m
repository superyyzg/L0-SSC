...
if (useSparse)
    ...
    opts.isreal = 1;
    opts.disp = 0;
    [V,ss] = eigs(L + 1e-8*speye(npix),nClusts,1,opts);
...
%for nRuns=1:50
    ...
    for tt=1:5   %% 10 iterations for kmeans
        ...
        if (distort<bestD && ~empty_cluster)   %% save result if better than the best so far
            bestD=distort;
            bestC=clusts;
        end
    end
